...
(async () => {
  ...
  try {
    ...
    await db.exec(fs.readFileSync(path.join(root, 'migrations/20260908_03_fluxo_professor.sql'), 'utf8'));
    await db.exec(fs.readFileSync(path.join(root, 'migrations/20260908_03_fluxo_professor.sql'), 'utf8'));
    await denied('professor nao edita prova enviada', 4, "UPDATE exams SET title='Alterada' WHERE id=$1", [exam]);
    await denied('professor nao exclui prova enviada', 4, 'DELETE FROM exams WHERE id=$1', [exam]);
    await asUser(3, "UPDATE exams SET review_status='em_revisao' WHERE id=$1", [exam]);
    await denied('professor nao retira prova em revisao', 4, "UPDATE exams SET review_status='rascunho' WHERE id=$1", [exam]);
    await asUser(3, "UPDATE exams SET review_status='devolvida',review_notes='Rever enunciado' WHERE id=$1", [exam]);
    await asUser(4, "UPDATE exams SET title='Corrigida' WHERE id=$1", [exam]);
    assert.equal((await asUser(4, 'SELECT review_notes FROM exams WHERE id=$1', [exam])).rows[0].review_notes, 'Rever enunciado'); checks++;
    await asUser(4, "UPDATE exams SET review_status='enviada' WHERE id=$1", [exam]);
