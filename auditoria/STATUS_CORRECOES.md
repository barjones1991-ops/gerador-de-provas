...
Atualizado em 08/09/2026. Primeiro lote implementado localmente; sem commit/push, aplicação de SQL no Supabase ou validação visual.
...
| S05, S06, S07 | Triggers validam papel/escola e estado inicial, protegem identidade e proprietário. Bootstrap por e-mail removido. Validação em PostgreSQL isolado; falta aplicar no ambiente real. |
...
| O03 | Incluídos testes funcionais, SQL real isolado, setup.html e pipeline de testes no GitHub Actions. A pipeline só executará no GitHub depois de publicação. |
...
- S08: escola proprietária permanente na prova, com migração e tratamento de transferências.
...
Seguir [o guia da migração](../migrations/README.md). Aplicar `migrations/20260908_01_seguranca.sql`, ativar confirmação de e-mail e validar os cinco perfis antes da publicação. A migração está incorporada a `setup_supabase.sql` para novas instalações.
...
Não houve inspeção visual nesta sessão: o inventário de ferramentas retornou nenhum navegador disponível. Não houve envio de e-mails de teste ou alterações no Supabase real.
