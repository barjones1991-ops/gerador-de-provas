...
assert.equal(app.document.getElementById('previewZoom').value,'fit');
 ...
 assert.equal(Safety.mathExpressionToLatex('(3^3)/5 ='),'\\frac{3^3}{5} =');
 assert.equal(Safety.mathExpressionToLatex('√(49/81) ='),'\\sqrt{\\frac{49}{81}} =');
 assert.equal(Safety.latexToMathExpression('\\sqrt{\\frac{49}{81}}'),'√(49/81)');
 ...
 const expressionButtons=[...expressionApp.document.querySelectorAll('.expression-editor-tools button')];
 assert(expressionButtons.some(button=>button.textContent==='aⁿ/b'));
 assert(expressionButtons.some(button=>button.textContent==='√(a/b)'));
